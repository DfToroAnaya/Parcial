...
<html lang="en">
  ...
  <body>
    ...
    <form method="POST" action="{{route('exhibition.update', ['exhibition' => $exhibitions->id])}}">
        @method('put')
        @csrf
        <div class="mb-3">
          <label for="id" class="form-label">Code</label>
          <input type="text" class="form-control" id="id" aria-describedby="idHelp"
          name="id" disabled='disabled' value="{{$exhibitions->id}}">
          ...
          <select class="form-select" id='works'  name='obra_id' required>
            ...
            @if ($work->id == $exhibitions->obra_id)
            <option selected value="{{$work->id}}">{{$work->título}}</option>
            @endif
